...
* 三人成团的拼团购买活动，单人购买XXX元、3人成团购买398元/人；开团人398元购买后需生成代表自己团的二维码海报，他人扫描二维码参加该团，3人即可成团，超过3人时需另开团。
...
|密码|单行输入框||必填项，密码格式与官网的密码格式相同|为空时提交，该输入框边框变为红色|1、输入账号对应的密码，密码显示为密文“原点”；</br>2、单行输入框默认显示“请输入密码”，输入框获取指针则该默认显示内容隐藏。|
|验证码|单行输入框+按钮||必填项，5字符|1、为空时提交，该输入框边框变为红色；</br>2、手机号码为已有账号，点击获取验证码则红色字提示“用户已存在”；|1、手机号码为非空时，点击“获取验证码”按钮，手机收到验证码短信；</br>2、验证码输入框，填写短信收到的信息；</br>3、单行输入框默认显示“输入验证码”，输入框获取指针则该默认显示内容隐藏。|
|点击注册|按钮||账号密码验证码正确，成功注册后弹出层页面变为“完善信息”（详见2.2.4.3）|1、密码不符合规则，则红色字提示“请填写6-16位，英文数字”（此部分如官网注册中有验证规则，与之相同即可）；</br>3、验证码错误，红色字则提示“验证码有误”。|点击注册，提交注册信息。|
...
### 2.4 活动到期后
* 团购购买时间为XX-XX，到达截止日期后，扫码参加活动打开购买页面，同时弹出层提示“拼团已结束，敬请期待”，点击该弹出层的确认按钮关闭弹出层，显示购买页面（购买页面的两个购买按钮变为灰色，无法点击的状态）
...
* 团购产生的订单，同时同步到官网（用户登录官网，可在订单中查看该订单信息）和订单系统（财务查看订单系统查看订单信息同时可操作退款）；
...
#### 3.2.2 订单详情
* 点击订单列表中的“查看”按钮，页面打开详情页，如下图：
* 顶部：订单状态（如为待支付，则倒计时自动关闭时间）；
* 订单信息：课程名称、姓名、电话、支付方式、金额；
* 交易信息：订单号、微信交易流水、订单提交时间、支付时间、取消时间；

![订单详情](https://raw.githubusercontent.com/systemcc/group-buying/master/IMG/%E5%BE%AE%E4%BF%A1-%E8%AE%A2%E5%8D%95%E8%AF%A6%E6%83%85.png)

## 4 后台统计
### 4.1 订单管理
* 点击开/参团后成功登录，即插入一条订单信息，支付或取消后更新订单相应信息；
* 订单信息同微信订单列表中的，增加检索条件。

![订单管理](https://raw.githubusercontent.com/systemcc/group-buying/master/IMG/PC-%E8%AE%A2%E5%8D%95%E7%AE%A1%E7%90%86.png)

### 4.2 活动用户管理
* 参加活动生成订单的所有用户列表。

![活动用户管理](https://raw.githubusercontent.com/systemcc/group-buying/master/IMG/%E6%B4%BB%E5%8A%A8%E7%94%A8%E6%88%B7%E7%AE%A1%E7%90%86.png)

#### 4.2.1 检索条件
|元素名称|类型|元素样式和说明|
|-------|----|-------------|
|学生姓名|单行输入框|精准检索，输入姓名检索内容|
|手机号码|单行输入框|精准检索，输入手机号码检索内容|
|OPENID|单行输入框|精准检索，输入订单信息人的OPENID检索内容|
|渠道来源|下拉框|选项包含：SA、创业CC、CS，检索某个渠道的所有用户|
|是否支付|下拉框|选项包含：是、否，检索成功支付或提交订单未支付的用户|
|是否老用户|下拉框|选项包含：是、否，根据ems中一个字段是否老用户，检索老用户或新用户|
|介绍人OpenID|单行输入框|精准检索，检索该OpenID为介绍人的所有用户|
|团ID|单行输入框|精准检索，检索属于该团的所有用户|
|检索|按钮|以上检索条件可任意组合使用，检索相应的信息|

#### 4.2.2 管理列表

|元素名称|元素样式和说明|
|-------|-------------|
|学生姓名|用户注册时，完善用户信息中填写的学生姓名|
|手机号码|注册或登录时使用的手机号码|
|OpenID|该用户使用的微信，授权后可获取OpenID信息|
|是否支付|1、是：参团并支付成功；</br>2、否：参团但尚未支付|
|是否老用户|根据字段，标记该用户是否为老用户|
|介绍人姓名|该用户扫描谁的二维码参加活动的，显示其姓名|
|介绍人ID|该用户扫描谁的二维码参加活动的，显示其OpenID|
|团ID|每个团长成功开团后，均记录一个团ID（唯一的），后续参团人员均记录在此团中，仅三人成团|
|渠道来源|活动海报二维码中均携带渠道编码参数，扫码参加活动，记录其渠道来源|
|表格属性|1、导出excel，可导出当前所有的检索结果（非本页数据）；</br>2、每页显示25行记录，翻页查看其他记录。|

### 4.3 转介绍奖励名单
* 统计转介绍数量的名单，每个参加并转发的用户均可以获得转介绍奖励。

![转介绍奖励名单](https://raw.githubusercontent.com/systemcc/group-buying/master/IMG/%E8%BD%AC%E4%BB%8B%E7%BB%8D%E5%A5%96%E5%8A%B1%E5%90%8D%E5%8D%95.png)
...
|OPENID|单行输入框|精准检索，输入订单信息人的OPENID检索内容|
|渠道来源|下拉框|选项包含：SA、创业CC、CS，检索某个渠道的所有用户|
|转介绍人数|单行输入框|填写数字，检索内容为转介绍人数等于或大于这个数字的所有用户|
|检索|按钮|以上检索条件可任意组合使用，检索相应的信息|
...
|手机号码|注册或登录时使用的手机号码|
|OpenID|该用户使用的微信，授权后可获取OpenID信息|
|转介绍人数|显示数字，表示多少人通过扫描该用户分享的二维码参加活动|
|渠道来源|活动海报二维码中均携带渠道编码参数，扫码参加活动，记录其渠道来源|
|表格属性|1、导出excel，可导出当前所有的检索结果（非本页数据）；</br>2、每页显示25行记录，翻页查看其他记录。|
